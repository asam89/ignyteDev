...
async def social_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "connect":
        ...
        if platform == "x":
            ...
            if len(context.args) < 3:
                await update.message.reply_text("Usage: `/social post linkedin <message>`")
                return
            message = " ".join(context.args[2:])
            response = await social_media_manager.post_linkedin(chat_id, message)
            await update.message.reply_text(response)

        elif platform == "instagram":
            if len(context.args) < 3:
                await update.message.reply_text("Usage: `/social post instagram <image_url> [caption]`")
                return
            image_url = context.args[2]
            caption = " ".join(context.args[3:]) if len(context.args) > 3 else ""
            response = await social_media_manager.post_instagram(chat_id, image_url, caption)
            await update.message.reply_text(response)
        ...
        await update.message.reply_text(f"Unknown social action: '{action}'. Use `connect` or `post`.")
