...
def call_gemini_sync(prompt: str) -> str:
    """Call Gemini synchronously — safe to run in a thread."""
...
def process_task_sync(task: str) -> dict:
    """
    Synchronous version — runs inside asyncio.to_thread.
    1. Read repo context
...
async def cmd_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        result = await asyncio.to_thread(process_task_sync, task)
        await update.message.reply_text(
            f"Done!\n\n"
        ...
        log.exception("Gemini returned invalid JSON")
        await update.message.reply_text(f"Gemini returned invalid JSON. Try rephrasing your task.\nError: {e}")
    except Exception as e:
...
if __name__ == "__main__":
    main()Autonomous Dev Node — Worker
Telegram Bot → Gemini Pro API → GitHub PR
