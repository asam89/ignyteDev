...
class SocialMediaManager:
    ...
    async def post_x(self, chat_id: int, text: str) -> str:
        """Post a tweet to X (Twitter)."""
        api = self._get_x_api(chat_id)
        if not api: # _get_x_api already logs the error
            return "❌ X (Twitter) account not connected or invalid tokens. Please connect first using `/social connect x <access_token> <access_token_secret>`."

        try:
            status = api.update_status(text)
            log.info(f"Successfully posted to X for chat_id {chat_id}: {text}")
            return f"✅ Tweet posted successfully! Link: https://twitter.com/{status.user.screen_name}/status/{status.id}"
        except tweepy.TweepyException as e:
            log.error(f"Failed to post to X for chat_id {chat_id}: {e}")
